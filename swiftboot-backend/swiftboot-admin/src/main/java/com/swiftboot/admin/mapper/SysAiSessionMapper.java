...
public interface SysAiSessionMapper extends BaseMapper<SysAiSession> {
    ...
    @Select("<script>" +
            ...
            "</script>")
    Map<String, Object> selectStats(@Param("timeRange") String timeRange, @Param("rankType") String rankType);
    ...
    @Select("<script>" +
            "SELECT " +
            "<choose>" +
            "  <when test='timeRange == \"month\"'>DATE_FORMAT(create_time, '%Y-%m-01')</when>" +
            "  <when test='timeRange == \"week\"'>DATE_FORMAT(DATE_SUB(create_time, INTERVAL WEEKDAY(create_time) DAY), '%Y-%m-%d')</when>" +
            "  <otherwise>DATE_FORMAT(create_time, '%Y-%m-%d')</otherwise>" +
            "</choose>" +
            " as date, SUM(tokens) as tokens " +
            "FROM sys_ai_session " +
            "WHERE 1=1 " +
            "<choose>" +
            "  <when test='timeRange == \"month\"'> AND create_time >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) </when>" +
            "  <when test='timeRange == \"week\"'> AND create_time >= DATE_SUB(CURDATE(), INTERVAL 6 WEEK) </when>" +
            "  <otherwise> AND create_time >= DATE_SUB(CURDATE(), INTERVAL 6 DAY) </otherwise>" +
            "</choose>" +
            "GROUP BY date " +
            "ORDER BY date" +
            "</script>")
    List<Map<String, Object>> selectTokenTrend(@Param("timeRange") String timeRange);
    ...
    @Select("<script>" +
            ...
            "  <otherwise>u.username, u.nickname, s.user_id, COALESCE(SUM(s.tokens), 0) as tokenConsumption</otherwise>" +
            "</choose>" +
            " FROM sys_ai_session s " +
            "LEFT JOIN sys_user u ON s.user_id = u.id " +
            "LEFT JOIN sys_dept d ON u.dept_id = d.id " +
            ...
            "</script>")
    List<Map<String, Object>> selectTopStats(@Param("timeRange") String timeRange, @Param("rankType") String rankType);
    ...
    @Select("<script>" +
            ...
            "</script>")
    Page<Map<String, Object>> selectUserTokenStats(Page<?> page, @Param("username") String username, @Param("timeRange") String timeRange, @Param("rankType") String rankType);
}
